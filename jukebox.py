...
PLAYLIST_LIMIT = int(os.getenv('PLAYLIST_LIMIT', '50'))  # Default to 10, -1 means no limit
...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def from_url(cls, url, *, loop=None, stream=False):
        ...
        try:
            ...
            raise Exception(f"Error extracting info from URL: {e}")
...
def get_playlist_entries(entries):
    """
...
async def ensure_voice(interaction: discord.Interaction) -> bool:
    ...
    if not interaction.guild.voice_client:
        ...
        try:
            ...
            logging.error(f"Failed to connect to voice channel {channel}: {e}", exc_info=True)
...
async def cmd_play(interaction: discord.Interaction, query: str):
    ...
    try:
        ...
        if 'entries' in data:
            # Playlist handling
            entries, total_count, was_limited = get_playlist_entries(data['entries'])
            added_count = 0
            ...
            if was_limited:
                await interaction.followup.send(f"Added {added_count} songs from playlist to queue (limited from {total_count} total songs)")
            else:
                await interaction.followup.send(f"Added {added_count} songs from playlist to queue")
        else:
            # Single song
            song_info = {
...
@bot.tree.command(name="playnow", description="Skip current song and play the specified song immediately")
async def cmd_playnow(interaction: discord.Interaction, query: str):
    ...
    try:
        ...
        if 'entries' in data:
            # For playlists, take only the first song for immediate play
            entries, total_count, was_limited = get_playlist_entries(data['entries'])
            ...
            # Add remaining playlist songs to the front of queue
            remaining_entries = entries[1:]  # Skip first song
            ...
            if len(remaining_entries) > 0:
                if was_limited:
                    await interaction.followup.send(f"Playing now: **{song_info['title']}**\nAdded {len(remaining_entries)} more songs from playlist to queue (limited from {total_count} total songs)")
                else:
        ...
        def after_playing(error):
            ...
            ctx_mock = MockCtx(interaction.guild, interaction.guild.voice_client, interaction.channel)
            asyncio.run_coroutine_threadsafe(play_next_auto(interaction.guild.id, interaction.channel), bot.loop)
...
async def cmd_playnext(interaction: discord.Interaction, query: str):
    ...
    try:
        ...
        if 'entries' in data:
            # Playlist handling - add in reverse order so first song plays first
            entries, total_count, was_limited = get_playlist_entries(data['entries'])
            added_count = 0
            ...
            if was_limited:
                await interaction.followup.send(f"Added {added_count} songs from playlist to play next (limited from {total_count} total songs)")
            else:
                await interaction.followup.send(f"Added {added_count} songs from playlist to play next")
        else:
            # Single song
            song_info = {
                'url': data['webpage_url'],
            ...
            queue.add(song_info, 'next')
            await interaction.followup.send(f"Added to play next: **{song_info['title']}**")
...
async def cmd_join(interaction: discord.Interaction):
    ...
    if interaction.guild.voice_client.channel != channel:
        try:
            ...
            logging.error(f"Failed to move to voice channel {channel}: {e}", exc_info=True)
            await interaction.response.send_message(f"Failed to move to voice channel: {str(e)}", ephemeral=True)
    else:
...
async def cmd_move(interaction: discord.Interaction, from_position: int, to_position: int):
    if not await ensure_guild(interaction):
        return
    ...
    if not (0 <= from_index < len(queue_list)) or not (0 <= to_index < len(queue_list)):
        await interaction.response.send_message(f"Invalid position! Queue has {len(queue_list)} songs (1-{len(queue_list)})", ephemeral=True)
        return
    
    if from_index == to_index:
        await interaction.response.send_message("Source and destination positions are the same!", ephemeral=True)
        return
    ...
    await interaction.response.send_message(f"✅ Moved **{song['title']}** from position {from_position} to position {to_position}")
...
async def cmd_remove(interaction: discord.Interaction, position: int):
    ...
    await interaction.response.send_message(f"❌ Removed **{removed_song['title']}** from position {position}")
...
async def play_next_auto(guild_id, channel):
    ...
    try:
        ...
        def after_playing(error):
            ...
            asyncio.run_coroutine_threadsafe(play_next_auto(guild_id, channel), bot.loop)
